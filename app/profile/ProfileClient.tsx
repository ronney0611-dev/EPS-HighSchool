...
const ProfilePage = () => {
    ...
    const totalMalade = allStudents.filter(s => s.status === 'malade').length
    const totalSpecial = allStudents.filter(s => s.status === 'special').length
                ...
                {classes.map((c, i) => (
                    <div key={i} className='bg-white text-black rounded-xl p-4 flex justify-between items-center'>
                        ...
                        <div className='flex gap-3 text-sm'>
                            <span className='bg-blue-200 px-2 py-1 rounded-full'>{(studentsByClass[c._id] || []).filter(s => s.gender === 'male').length} ذكر</span>
                            <span className='bg-pink-200 px-2 py-1 rounded-full'>{(studentsByClass[c._id] || []).filter(s => s.gender === 'female').length} أنثى</span>
                            <span className='bg-gray-200 px-2 py-1 rounded-full'>{(studentsByClass[c._id] || []).length} مجموع</span>
                        </div>
                    </div>
                ))}
            </div>
